        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {/* Touch-only camera shortcut: `capture` opens the camera directly
                  and is ignored on desktop, so this is hidden there (.touch-only). */}
              <button
                type="button"
                onClick={() => !submitting && cameraInputRef.current?.click()}
                disabled={submitting}
                className="touch-only mt-2 w-full items-center justify-center gap-1.5 border border-emerald-200 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg px-4 py-2 text-sm font-medium transition-colors disabled:opacity-50"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
