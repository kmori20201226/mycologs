...
usage() {
  echo "Usage: $0 {start|stop|restart|status|logs|migrate|seed-taxonomy|seed-admin|make-test-user|build|psql|maintenance|list-user|backup-posts|restore-posts}"
  echo ""
...
cmd_seed_plans() {
  echo "Seeding plans from Stripe (test mode)..."
